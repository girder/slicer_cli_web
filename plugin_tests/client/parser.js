...
describe('XML Schema parser', function () {
    describe('type conversion', function () {
        it('number', function () {
            var type = 'number';
            expect(slicer.schema.convertValue(type, '1')).toBe(1);
        });
        it('string', function () {
            var type = 'string';
            expect(slicer.schema.convertValue(type, '1')).toBe('1');
        ...
        it('boolean', function () {
            var type = 'boolean';
            expect(slicer.schema.convertValue(type, 'true')).toBe(true);
            expect(slicer.schema.convertValue(type, 'false')).toBe(false);
            expect(slicer.schema.convertValue(type, '')).toBe(false);
        });
        it('string-vector', function () {
            var type = 'string-vector';
            expect(slicer.schema.convertValue(type, '1,2,3')).toEqual(['1', '2', '3']);
        });
        ...
        it('number-enumeration', function () {
            ...
            expect(slicer.schema.convertValue(type, '1')).toEqual(1);
        });
    ...
    describe('constraints', function () {
        it('empty', function () {
            expect(slicer.schema._parseConstraints('number'))
                .toEqual({});
        });
        it('missing step', function () {
            ...
            expect(slicer.schema._parseConstraints(
                'number',
                xml
            )).toEqual({min: 1, max: 3});
        });
    ...
    describe('parameters', function () {
        it('integer', function () {
            ...
            expect(slicer.schema._parseParam(
    ...
    describe('default value', function () {
        it('no value provided', function () {
            expect(slicer.schema._parseDefault('integer', $())).toEqual({});
        });
        it('a integer provided', function () {
            expect(slicer.schema._parseDefault(
                'integer',
                $('<default>1</default>')
            )).toEqual({value: '1'});
        });
